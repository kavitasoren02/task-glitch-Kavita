...
export function useTasks(): UseTasksState {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        // Ensure final data is validated by `normalizeTasks` and generated fallback;
        // avoid appending unvalidated/malformed rows here.
  ...
  // Removed opportunistic secondary fetch that could append duplicate tasks
  // The initial load() handles fetching and normalization deterministically above.
